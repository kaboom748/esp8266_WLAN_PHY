...
// ============================================================
// ESP8266 OOK v32.18 - 3500 BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + fixed RX gain 0x7C03
//
// Goals:
//   * no session, no peer, no ACK, no CSMA
//   * APWR deliberately left at 255 as requested
//   * 3500 bit/s OOK raw line rate with exact fractional TX/RX scheduling
...
// 3500 raw OOK bits/s. Keep the proven 8 samples/bit RH_ASK PLL, but
// schedule both TX and RX fractionally so there is no integer-us drift.
// TX bit period = 1e6/3500 = 285.714... us -> 285/286 us pattern.
// RX tick period = 1e6/(3500*8) = 35.714... us -> 35/36 us pattern.
static constexpr uint16_t BIT_RATE            = 3500u;
...
namespace Phy {
  ...
  static uint32_t measureE4() {
    ...
    return 100000;
#else
    // FAST E4: keep IQ_EST enabled between RX samples and re-arm only START.
    // Freshness is guaranteed by the silicon-validated DONE 1->0->1 handshake:
    //   START=0 -> wait DONE=0 -> START=1 -> wait DONE=1 -> read E4.
    // The first sample after boot/TX uses the ROM enable path to program MODE/N.
    static constexpr uint32_t IQ_CTRL       = 0x6000057Cu;
    static constexpr uint32_t IQ_DONE_MASK  = 0x80000000u;
    static constexpr uint32_t IQ_START_MASK = 0x00000002u;
    static constexpr uint32_t IQ_EN_MASK    = 0x00000001u;
    static constexpr uint32_t TIMEOUT_CYCLES = (F_CPU / 1000000UL) * 25UL;

    auto ccount = []() -> uint32_t {
      uint32_t c;
      __asm__ volatile("rsr.ccount %0" : "=a"(c));
      return c;
    };

    uint32_t ctrl = rd32(IQ_CTRL);

    // enterTx() disables IQ_EST. Detect that state here so no other code path
    // needs to change: arm once with the known ROM routine, then keep it hot.
    if ((ctrl & IQ_EN_MASK) == 0u) {
      memw(); iqEnable(1u, IQ_N); memw();
      return rd32(IQ_POWER_E4);
    }

    const uint32_t base = ctrl & ~(IQ_DONE_MASK | IQ_START_MASK);

    // End the previous acquisition and prove DONE really returned low.
    wr32(IQ_CTRL, base); memw();
    uint32_t t0 = ccount();
    while (rd32(IQ_CTRL) & IQ_DONE_MASK) {
      if ((uint32_t)(ccount() - t0) >= TIMEOUT_CYCLES) goto rom_fallback;
    }

    // Start a new acquisition and wait for its own completion.
    wr32(IQ_CTRL, base | IQ_START_MASK); memw();
    t0 = ccount();
    while ((rd32(IQ_CTRL) & IQ_DONE_MASK) == 0u) {
      if ((uint32_t)(ccount() - t0) >= TIMEOUT_CYCLES) goto rom_fallback;
    }
...
rom_fallback:
    // Exceptional recovery only: restore the proven ROM state machine.
    memw(); iqDisable(); memw();
    memw(); iqEnable(1u, IQ_N); memw();
    return rd32(IQ_POWER_E4);
#endif
...
// ============================================================
// TX: RH_ASK style symbol stream at exact-average 3500 raw bit/s
// ============================================================
...
void setup() {
  ...
  delay(150);
  Serial.println();
  Serial.println("ESP8266 OOK v32.18 3500BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + FIXED GAIN 0x7C03 + 8X PLL + 4b6b + CRC16");
  ...
  scheduleNextTx();
  Serial.println("READY 3500bps: IQ_N=192; single E4/tick; DATA threshold frozen after START; asymmetric DATA hysteresis rise=T fall=T-h; trimmed analog 4b6b ML rescue + c/12 confidence gate; FAILBLOCK E4 microscope enabled; gain fixed 0x7C03");
  printDiagnostics();
}
