...
## Consolidated Reverse-Engineering Project Reference — v2.0

**Consolidation date: 2026-09-13**  
**Status: CONSOLIDATED REFERENCE — software/ROM + hardware boundaries + separate RF validation**  
...
> **Normative rule.** Parts I through XII of this document form the normative v2.0 synthesis. The appendices preserve the four complete v1.0 references and the QAM v0.60→v0.83 history so that no source information is lost. Where historical wording differs from the v2.0 synthesis, the v2.0 synthesis takes precedence.
...
This level requires actual measurement: dBm, MMIO→RF delay, settling, phase, jitter, sensitivity, BER, gain in dB, the `tone_control→Hz` law, freshness of CFO on a non-802.11 tone, E4 response to an external carrier, etc.
...
|---|---:|---:|---|
| OOK | **100%** | **~98–100% depending on the canonical scope** | generation and IQ_EST energy detector understood; RF measurements remain open |
| ASK / M-ASK | **100%** | **~95–100% depending on the canonical scope** | fixed gain + E4 + thresholds; actual dynamic range remains to be measured |
...
0x60009824              hardware noise-floor readout
...
DONE
   ↓
0x600005E4 = window energy/power
   ↓
1 threshold: OOK
...
The wrappers historically named `phy_enable_agc()` / `phy_disable_agc()` must not be interpreted as proof of general AGC control in this corpus. Tracing the operation table shows that, in the table installed by `register_phy_ops()`, they correspond to **CCA**.

For ASK/M-ASK, the genuinely useful fixed-gain path is PBUS (`rom_pbus_set_rxgain`, `pbus_set_rxbbgain`, RX gain tables).

## 15. Physical limit

The external-input→IQ_EST software path is very strongly reconstructed, but the RF characteristics still require measurement: `E4→dBm`, `N→time`, sensitivity, saturation, gain-code→dB, and the actual number of distinguishable ASK levels.
...
Remaining questions are essentially physical:
...
```text
OOK/ASK : software/ROM layer practically closed
...
> The following appendices are included to satisfy the **no information loss** requirement. They preserve the content of the four v1.0 references, with only heading-level demotion for Markdown integration. Their historical content may use wording that is later refined by the normative v2.0 synthesis above.
