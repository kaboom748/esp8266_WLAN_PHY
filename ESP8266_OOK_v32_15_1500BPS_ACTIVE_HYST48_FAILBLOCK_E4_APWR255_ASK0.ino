...
// ============================================================
// ESP8266 OOK v32.15 - 1500 BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + fixed RX gain 0x7C03
//
// Goals:
...
public:
  ...
  void addSample(uint32_t tUs, uint32_t e) {
    ...
    // v32.15: keep the proven v32.11 slicer behavior; add E4-only failure diagnostics.
...
namespace Phy {
  ...
  static uint32_t measureE4() {
    ...
    return 100000;
#else
    // v32.15 baseline: one E4 integration per PLL tick with IQ_N=192; DATA threshold frozen after START.
...
static void printPendingFailureDiag() {
  ...
  Serial.println();
  Serial.printf("  slicer thr=%lu hyst=%lu effectiveLOW=%lu effectiveHIGH=%lu\n",
...
void setup() {
  ...
  delay(150);
  Serial.println();
  Serial.println("ESP8266 OOK v32.15 1500BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + FIXED GAIN 0x7C03 + 8X PLL + 4b6b + CRC16");
  ...
  Serial.println("RX E4: single IQ_N=192 integration/tick; FAILBLOCK logs bits + votes + phase + sample gaps + E4 avg/min/max + frozen threshold + active hyst c/48");
  ...
  scheduleNextTx();
  Serial.println("READY 1500bps: IQ_N=192; single E4/tick; DATA threshold frozen after START; FAILBLOCK E4 microscope enabled; gain fixed 0x7C03");
  printDiagnostics();
}
