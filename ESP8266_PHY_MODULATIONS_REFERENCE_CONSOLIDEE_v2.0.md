...
## Référence consolidée du projet de reverse-engineering — v2.0

**Date de consolidation : 2026-09-13**  
**Statut : RÉFÉRENCE CONSOLIDÉE — logiciel/ROM + frontières hardware + validations RF séparées**  
...
Ce niveau exige une mesure réelle : dBm, délai MMIO→RF, settling, phase, jitter, sensibilité, BER, gain en dB, loi `tone_control→Hz`, fraîcheur d'un CFO sur tone non-802.11, réponse d'E4 à une porteuse externe, etc.
...
|---|---:|---:|---|
| OOK | **100 %** | **~98–100 % selon le périmètre canonique** | génération et détecteur d'énergie IQ_EST compris ; mesures RF ouvertes |
| ASK / M-ASK | **100 %** | **~95–100 % selon le périmètre canonique** | gain fixe + E4 + seuils ; dynamique réelle à mesurer |
...
0x60009824              readout noise-floor hardware
...
DONE
   ↓
0x600005E4 = énergie/power sur fenêtre
   ↓
1 seuil : OOK
...
Les wrappers historiquement nommés `phy_enable_agc()` / `phy_disable_agc()` ne doivent pas être interprétés comme un contrôle AGC général dans ce corpus. Le traçage de la table d'opérations montre qu'ils correspondent au **CCA** dans la table installée par `register_phy_ops()`.

Pour ASK/M-ASK, la voie de gain fixe réellement utile est PBUS (`rom_pbus_set_rxgain`, `pbus_set_rxbbgain`, tables de gain RX).

## 15. Limite physique

Le chemin logiciel externe→IQ_EST est très fortement reconstruit, mais les caractéristiques RF restent à mesurer : `E4→dBm`, `N→temps`, sensibilité, saturation, gain-code→dB et nombre réel de niveaux ASK séparables.
...
Questions restantes essentiellement physiques :
...
```text
OOK/ASK : couche logicielle/ROM pratiquement fermée
