...
// ============================================================
// ESP8266 OOK v32.18 - 2000 BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + fixed RX gain 0x7C03
//
// Goals:
//   * no session, no peer, no ACK, no CSMA
//   * APWR deliberately left at 255 as requested
//   * 2000 bit/s OOK raw line rate with exact fractional TX/RX scheduling
...
// 2000 raw OOK bits/s. Keep the proven 8 samples/bit RH_ASK PLL, but
// schedule both TX and RX fractionally so there is no integer-us drift.
// TX bit period = 1e6/2000 = 500 us exactly.
// RX tick period = 1e6/(2000*8) = 62.5 us -> 62/63 us pattern.
static constexpr uint16_t BIT_RATE            = 2000u;
...
// ============================================================
// TX: RH_ASK style symbol stream at exact-average 2000 raw bit/s
// ============================================================
...
void setup() {
  ...
  delay(150);
  Serial.println();
  Serial.println("ESP8266 OOK v32.18 2000BPS + IQ_N=192 + FROZEN DATA THRESHOLD + FAILBLOCK E4 DIAG + RESTORED TX + PREAMBLE QUALIFIER + FIXED GAIN 0x7C03 + 8X PLL + 4b6b + CRC16");
  ...
  scheduleNextTx();
  Serial.println("READY 2000bps: IQ_N=192; single E4/tick; DATA threshold frozen after START; asymmetric DATA hysteresis rise=T fall=T-h; trimmed analog 4b6b ML rescue + c/12 confidence gate; FAILBLOCK E4 microscope enabled; gain fixed 0x7C03");
  printDiagnostics();
}
